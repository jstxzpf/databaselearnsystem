...
2. 难度适中，符合本科生水平
3. 题目表述清晰、准确
4. 请以JSON格式返回结果，是一个包含题目对象的数组。每个题目对象包含以下字段：
   - content: 题目内容（如果是选择题，不包含选项）
   - options: 选项数组（如果是选择题，包含["A. xxx", "B. xxx", "C. xxx", "D. xxx"]，非选择题为空数组）
   - answer: 正确答案（选择题为选项字母，如"A"，非选择题为参考答案）
   - analysis: 答案解析或考点说明

JSON格式示例：
[
    {{
        "content": "关于数据库系统的特点，下列说法错误的是？",
        "options": ["A. 数据结构化", "B. 数据共享性高", "C. 数据独立性差", "D. 数据由DBMS统一管理"],
        "answer": "C",
        "analysis": "数据库系统的主要特点包括数据结构化、数据的共享性高、数据的独立性高（包括物理独立性和逻辑独立性）、数据由DBMS统一管理和控制。"
    }}
]

请直接返回JSON数组，不要包含Markdown代码块标记（如```json），也不要包含其他文字说明。
"""
